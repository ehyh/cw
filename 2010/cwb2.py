...
# Description: Read resource info from RESOURCE.DAT,
#              get and validate extra details based on resource type,
#              write to URESOURCE.DAT
            ...
            if resource_type == "C":
                ...
                while not valid_num_tracks:
                    ...
                    if len(num_tracks) == 0: # presence check
                        ...
                        print("Invalid! Must be below 20. Try again.")
                    else:
...
##                                                                 + resource.getArtist() + getNoOfTracks() + "\n")
##
##                else: 
##                    uresource_file.write(resource.getResourceNo() + resource.getTitle() + resource.getDateAcquired() + resource.getResourceType() + "NULL(Count to 50)" + "00" \
##                                                                 + resource.getDirector() + getRunningTime() + "\n")

            # a better way
        for resource in resource_list:
            uresource_file.write(resource.display()) # polymorphism
